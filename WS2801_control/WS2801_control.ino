...
void post_frame (void) {
  ...
  long this_led_color = strip_colors[0]; //for debug
  ...
  for(int LED_number = 0 ; LED_number < STRIP_LENGTH ; LED_number++) {
    if(LED_number%2==0)
        this_led_color = (counter<<8); //24 bits of color data
    else
        this_led_color = ((280-counter) << 0); //24 bits of color data
  ...
  //delayMicroseconds(250); //Wait for 500us to go into reset
}
